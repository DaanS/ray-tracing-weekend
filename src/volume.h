...
struct isotropic : public material {
    ...
    virtual std::tuple<bool, color, ray> scatter(ray const& r, hit_record const& h) const override {
        return {true, albedo->value(h.u, h.v, h.p), ray(h.p, vec3_random_sphere(), r.time)};
    }
